...
class Wallet {
    ...
    static create(props) {
        ...
        const clientIdResult = ClientId_1.ClientId.create(props.clientId);
        if (clientIdResult.isFailure) {
            return Result_1.Result.fail(`Invalid client ID: ${clientIdResult.getError()}`);
        }
        return Result_1.Result.ok(new Wallet(walletIdResult.getValue(), clientIdResult.getValue(), walletTypeResult.getValue(), currencyResult.getValue(), walletLimitResult.getValue(), new Date(), new Date()));
    }
}
